...
class TestReflectionBot(unittest.TestCase):
    ...
    def test_analyze_and_store_persistence(self):
        ...
        self.assertTrue(
            any(
                insight.trade_id == self.trade_outcome.trade_id for insight in retrieved
            ),
    ...
    def test_reflection_insights_persistence(self):
        ...
        print(
            f"[DEBUG] MIN_REFLECTION_PNL: 10.0, test pnl_percent: {trade.pnl_percent}"
        )
        ...
        self.assertTrue(
            any(insight.trade_id == trade.trade_id for insight in retrieved)
        )
