...
class TradeExecutorBot(LoggerMixin):
    """
    ...
    def __init__(self, api_key, api_secret, paper_trading=True):
        ...
        self.api = tradeapi.REST(
            ...
            base_url=URL(
                ALPACA_BASE_URL
            ),  # Trading endpoint (paper or live) from config_system.py
    ...
    def execute_trade(self, symbol, side, quantity, confidence):
        ...
        self.logger.debug(
            f"{self.__module__.split('.')[-1]} [{self.__class__.__name__}]: [{method}(symbol='{symbol}', side='{side}', quantity={quantity}, confidence={confidence})] START"
        )
        asset_type = "crypto" if "/" in symbol else "stock"
        self.logger.info(
            f"[TradeExecutorBot] Received trade request: symbol={symbol}, side={side}, quantity={quantity}, confidence={confidence}, asset_type={asset_type}"
        )
        try:
            # === INPUT VALIDATION ===
            ...
            if not all([symbol, side, quantity]):
                raise ValueError(
                    "Missing required parameters: symbol, side, and quantity must all be provided"
                )
            ...
            order = {
                ...
                "time_in_force": (
                    "gtc" if asset_type == "crypto" else "day"
                ),  # Fix: 'gtc' for crypto, 'day' for stocks
            }

            # Log the order attempt for audit purposes (no emoji)
            self.logger.info(
                f"[TradeExecutorBot] Attempting to execute {side} order for {symbol} ({asset_type}): qty={quantity}, confidence={confidence}"
            )
            ...
            try:
                ...
                self.logger.info(
                    f"[TradeExecutorBot] Order placed successfully: {symbol} {side} {quantity} ({asset_type})"
                )
                ...
                self.logger.error(
                    f"[TradeExecutorBot] Trade execution failed for {symbol} ({asset_type}): {e}"
                )
                self.logger.error(
                    "Returning False, str(e) due to trade execution failure."
                )
                return False, str(e)
            ...
            self.logger.error(
                "Returning error dict due to unexpected error in execute_trade."
            )
            return {"error": f"Unexpected error executing trade for {symbol}: {str(e)}", "status": "failed"}, None
        finally:
            self.logger.debug(
                f"{self.__module__.split('.')[-1]} [{self.__class__.__name__}: [{method}(symbol='{symbol}', action='{side}', quantity={quantity}, price={confidence})] END"
            )
    ...
    def get_open_positions(self, asset_type=None):
        ...
        try:
            ...
            for pos in positions:
                ...
                atype = (
                    "crypto"
                    if ("/" in symbol) or (asset_class == "crypto")
                    else "stock"
                )
    ...
    def cross_check_order_history(self, db_bot=None, days=7, log_only=True):
        ...
        alpaca_set = set(
            (o["symbol"], o["side"], o["qty"], o["submitted_at"]) for o in alpaca_orders
        )
        internal_set = set(
            (t["symbol"], t["trade_type"], t["quantity"], t["timestamp"])
        ...
        if not discrepancies:
            self.logger.info(
                "[ORDER_XCHECK] No discrepancies found between Alpaca and internal records."
            )
        if log_only:
            return None
    ...
    def selftest() -> bool:
        """
        Run a self-test to verify trade execution logic. Returns True if successful.

        This method performs a series of automated tests on the trading logic to ensure
        all components are functioning correctly. It uses mock objects to simulate
        Alpaca API and database interactions, allowing for safe testing without
        real trades or data modifications.

        Test Cases:
        1. Successful trade execution
        2. Failed trade execution (simulated failure)
        3. Account information retrieval
        4. Order history fetching
        5. Cross-checking order history with internal records

        Each test case checks the relevant functionality and asserts the expected
        outcomes. Any discrepancies or failures are reported via assertions.
        ...
        class MockAPI:
            ...
            def list_orders(self, status="all", limit=50):
                ...
                return [
                    Order("AAPL", "buy", 1, "2023-01-01T00:00:00Z"),
                    Order("TSLA", "sell", 2, "2023-01-02T00:00:00Z"),
                ]
        ...
        try:
            ...
            assert (
                success and resp["symbol"] == "AAPL"
            ), f"Expected success for AAPL, got {success}, {resp}"
            ...
            assert (
                isinstance(acct, dict) and "cash" in acct and "buying_power" in acct
            ), "Account info missing expected keys."
            print("    -> Account info logic passed.")  # CLI/test output
            # Test order history fetch
            orders = bot.fetch_order_history()
            assert (
                isinstance(orders, list) and len(orders) == 2
            ), "Order history fetch failed"
            print("    -> Order history fetch logic passed.")
            # Test cross-check logic
            discrepancies = bot.cross_check_order_history(
                db_bot=MockDB(), log_only=False
            )
            if discrepancies is None:
                discrepancies = []
            assert (
                isinstance(discrepancies, list) and len(discrepancies) == 2
            ), f"Expected 2 discrepancies, got {len(discrepancies)}"
            print("    -> Order history cross-check logic passed.")
            print("--- TradeExecutorBot Self-Test PASSED ---")
