...
class DatabaseBot:
    ...
    def save_analysis(self, symbol, asset_type, analysis, current_price, context_data=None):
    ...
    def store_reflection_insight(self, symbol: str, trade_id: str, original_analysis_id: int,
                               entry_price: float, exit_price: float, pnl: float,
                               hold_duration_hours: float, market_conditions: str,
                               ai_reflection: str, key_insights: str, lessons_learned: str,
                               confidence_accuracy: float) -> int:
        ...
        with sqlite3.connect(self.db_file) as conn:
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                datetime.now().isoformat(), symbol, trade_id, original_analysis_id,
                entry_price, exit_price, pnl, pnl_percentage, hold_duration_hours,
                market_conditions, ai_reflection, key_insights, lessons_learned,
                confidence_accuracy
            ))
    ...
    def get_cross_asset_insights(self, asset_type: Optional[str] = None, days: int = 30) -> List[Dict]:
    ...
    def store_screening_results(self, market_sentiment: str, market_volatility: float,
                              risk_environment: str, selected_assets: List[str],
                              screening_scores: Dict[str, float], ai_insights: str,
                              top_sectors: List[str]) -> int:
    ...
    def store_analysis_context(self, symbol: str, context_type: str, context_data: Dict,
                             relevance_score: float = 1.0, expires_hours: Optional[int] = None) -> int:
    ...
    def get_analysis_context(self, symbol: str, context_types: Optional[List[str]] = None, days = 30, limit = 20) -> List[Dict]:
    ...
    def store_trade_outcome(self, symbol: str, asset_type: str, trade_type: str,
                          entry_price: float, exit_price: float, quantity: float,
                          hold_duration_minutes: int, original_confidence: float,
                          analysis_id: int, execution_status: str, fees: float = 0.0) -> int:
        ...
        with sqlite3.connect(self.db_file) as conn:
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                datetime.now().isoformat(), symbol, asset_type, trade_type,
                entry_price, exit_price, quantity, pnl, pnl_percentage,
                hold_duration_minutes, original_confidence, analysis_id,
                execution_status, fees
            ))
            return cursor.lastrowid or 0

    def get_trade_outcomes(self, symbol: Optional[str] = None, days: int = 30) -> List[Dict]:
    ...
    def get_comprehensive_performance(self, symbol: Optional[str] = None, days: int = 30) -> Dict[str, Any]:
        ...
        avg_pnl_percentage = sum(t['pnl_percentage'] for t in trade_outcomes) / total_trades if total_trades > 0 else 0
        ...
        return {
            ...
            'avg_confidence_accuracy': sum(i['confidence_accuracy'] for i in insights) / len(insights) if insights else 0,
            'key_lessons': [i['lessons_learned'] for i in insights[:3]]  # Top 3 recent lessons
        }
