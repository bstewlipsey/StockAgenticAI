...
def test_asset_screener_fallback_always_returns_crypto():
    ...
    assert len(crypto_assets) >= 1, "Fallback must always include at least one crypto asset"
...
def test_asset_screener_screen_assets_always_returns_crypto(monkeypatch):
    ...
    monkeypatch.setattr(screener, '_analyze_market_overview', lambda: type('Dummy', (), {'market_sentiment': 'neutral', 'risk_environment': 'medium', 'top_sectors': [], 'market_volatility': 20.0, 'ai_insights': ''})())
    results = screener.screen_assets()
    crypto_assets = [r for r in results if getattr(r, 'asset_type', None) == 'crypto']
    assert len(crypto_assets) >= 1, "screen_assets must always return at least one crypto asset"
    assert all(isinstance(r, AssetScreeningResult) for r in results)
