...
class IndicatorBot(LoggerMixin):
    """
    Bot for calculating technical indicators and generating trading signals.
    Provides modular indicator calculation for use in trading agents.
    """

    def __init__(self, prices, window=SMA_WINDOW):
        """
        Initialize IndicatorBot with price data and window size.
        Sets up DataFrame for indicator calculations.
        """
        super().__init__()
    ...
    def calculate_indicators(self):
        """
        Calculate all supported technical indicators for the price data.
        Uses caching for efficiency. Returns a dict of indicator values.
        Handles minimum data requirements for each indicator.
        """
        ...
        try:
            ...
            # RSI
            rsi_indicator = (
                RSIIndicator(close=self.df["close"], window=14)
                if data_length >= min_rsi_periods
                else None
            )
            # MACD
            macd_indicator = (
                MACD(close=self.df["close"])
                if data_length >= min_macd_periods
                else None
            )
            # SMA
            sma_indicator = (
                SMAIndicator(close=self.df["close"], window=self.window)
                if data_length >= min_sma_periods
                else None
            )
            # EMA
            ema_indicator = (
                EMAIndicator(close=self.df["close"], window=self.window)
                if data_length >= min_ema_periods
                else None
            )
            # Bollinger Bands
            bb_indicator = (
                BollingerBands(close=self.df["close"], window=self.window)
            ...
            # Extract values
            indicators["rsi"] = (
                rsi_indicator.rsi().iloc[-1] if rsi_indicator is not None else 50.0
            )
            indicators["macd"] = (
                macd_indicator.macd().iloc[-1] if macd_indicator is not None else 0.0
            )
            ...
            indicators["sma_20"] = (
                sma_indicator.sma_indicator().iloc[-1]
                if sma_indicator is not None
                else (self.df["close"].iloc[-1] if len(self.df) > 0 else 0.0)
            )
            indicators["ema_20"] = (
                ema_indicator.ema_indicator().iloc[-1]
            ...
            indicators["obv"] = (
                obv_indicator.on_balance_volume().iloc[-1]
                if obv_indicator is not None
                else 0.0
            )
            indicators["adi"] = (
                adi_indicator.acc_dist_index().iloc[-1]
                if adi_indicator is not None
                else 0.0
            )
            indicators["vwap"] = (
                vwap_indicator.volume_weighted_average_price().iloc[-1]
                if vwap_indicator is not None
                else (self.df["close"].iloc[-1] if len(self.df) > 0 else 0.0)
            )
            indicators["current_price"] = (
                self.df["close"].iloc[-1] if len(self.df) > 0 else 0.0
            )
    ...
    def selftest():
        ...
        try:
            ...
            print(
                f"  RSI: {indicators.get('rsi', 'N/A'):.2f}, MACD: {indicators.get('macd', 'N/A'):.4f}"
            )
            return True
