...
class ReflectionBot(LoggerMixin):
    """
    ...
    def __init__(self):
        """
        Initialize the ReflectionBot and required dependencies.
        Sets up parameters for reflection thresholds and batch size.
        """
        super().__init__()
        ...
        self.min_reflection_pnl = (
            MIN_REFLECTION_PNL  # Only reflect on trades with >2% impact
        )
        self.max_reflection_age_days = (
            MAX_REFLECTION_AGE_DAYS  # Only reflect on trades within 30 days
        )
        self.reflection_batch_size = 5  # Process 5 trades at a time

        # Debug log for reflection threshold
        logger.info(
            f"[DEBUG] ReflectionBot MIN_REFLECTION_PNL: {self.min_reflection_pnl}"
        )
    ...
    def analyze_completed_trade(
        ...
        """
        logger.info(
            f"Analyzing completed trade: {trade_outcome.symbol} (P&L: {trade_outcome.pnl_percent:.1f}%)"
        )
        # --- LOG: Full trade_outcome sent to ReflectionBot ---
        logger.info(f"[REFLECTION_INPUT] trade_outcome={trade_outcome}")
        insights = []
        method = "analyze_completed_trade"
        try:
            self.logger.debug(f"{self.__module__.split('.')[-1]} [{self.__class__.__name__}]: [{method}(symbol='{getattr(trade_outcome, 'symbol', None)}')] START")
            # === STEP 1: DETERMINE REFLECTION PRIORITY ===
            should_reflect = self._should_reflect_on_trade(trade_outcome)
            if not should_reflect:
                logger.info(
                    f"Skipping reflection for {trade_outcome.symbol} - below reflection threshold"
                )
                return insights
            # === STEP 2: GATHER CONTEXT FOR REFLECTION ===
            reflection_context = self._gather_reflection_context(trade_outcome)
            logger.info(f"[REFLECTION_CONTEXT] {trade_outcome.symbol} | context={reflection_context}")
            # === STEP 3: GENERATE AI-POWERED INSIGHTS ===
            ai_insights = self._generate_ai_reflection(
                trade_outcome, reflection_context
            )
            ...
            # --- LOG: Full list of generated insights ---
            logger.info(f"[REFLECTION_OUTPUT] {trade_outcome.symbol} | insights={insights}")
            logger.info(
                f"Generated {len(insights)} insights for {trade_outcome.symbol}"
            )
            self.logger.debug(f"{self.__module__.split('.')[-1]} [{self.__class__.__name__}]: [{method}(symbol='{getattr(trade_outcome, 'symbol', None)}')] END")
            return insights
        except Exception as e:
            logger.error(f"Error analyzing trade {trade_outcome.trade_id}: {e}")
            return insights

    def batch_analyze_recent_trades(
        ...
        try:
            ...
            logger.info(
                f"Batch reflection complete: {len(all_insights)} total insights from {len(completed_trades)} trades"
            )
            return all_insights
    ...
    def _should_reflect_on_trade(self, trade_outcome: TradeOutcome) -> bool:
        ...
        logger.info(
            f"[DEBUG] _should_reflect_on_trade: abs({trade_outcome.pnl_percent}) < {self.min_reflection_pnl} (type: {type(trade_outcome.pnl_percent)})"
        )
        ...
        if abs(pnl_percent) < self.min_reflection_pnl:
            logger.info(
                f"Reflection threshold check: abs({pnl_percent}) < {self.min_reflection_pnl}"
            )
            return False
        logger.info(
            f"Reflection threshold check: abs({pnl_percent}) <= {self.min_reflection_pnl}"
        ...
        existing_insights = [
            insight
            for insight in self.database_bot.get_reflection_insights(
        ...
        try:
            ...
            if trade_outcome.market_conditions_entry:
                context["market_conditions"][
                    "entry"
                ] = trade_outcome.market_conditions_entry
            if trade_outcome.market_conditions_exit:
                context["market_conditions"][
                    "exit"
                ] = trade_outcome.market_conditions_exit

            # Get historical performance for this symbol (recent trades only)
            historical_trades = self.database_bot.get_trade_outcomes(
                symbol=trade_outcome.symbol, days=30
            )
            if historical_trades:
                win_rate = len(
                    [t for t in historical_trades if t.get("pnl", 0) > 0]
                ) / len(historical_trades)
                avg_return = sum(
                    t.get("pnl_percentage", 0) for t in historical_trades
                ) / len(historical_trades)
            ...
            for t in all_trades:
                ...
                if entry_time and exit_time:
                    if (
                        entry_time <= trade_outcome.exit_time
                        and exit_time >= trade_outcome.entry_time
                    ):
    ...
    def _generate_ai_reflection(
        self, trade_outcome: TradeOutcome, context: Dict[str, Any]
    ) -> List[ReflectionInsight]:
        ...
        try:
            ...
            if isinstance(ai_response, dict) and "reasoning" in ai_response:
                ...
                ai_insights = self._parse_ai_reflection_response(
                    ai_response, trade_outcome
                )
                insights.extend(ai_insights)
    ...
    def _parse_ai_reflection_response(
        self, ai_response: Dict[str, Any], trade_outcome: TradeOutcome
    ) -> List[ReflectionInsight]:
        ...
        try:
            ...
            if trade_outcome.original_analysis:
                original_confidence = trade_outcome.original_analysis.get(
                    "confidence", 0
                )
    ...
    def _perform_systematic_analysis(
        self, trade_outcome: TradeOutcome
    ) -> List[ReflectionInsight]:
    ...
    def _analyze_cross_trade_patterns(
        ...
        try:
            ...
            if profitable_trades and losing_trades:
                avg_profit = sum(t.pnl_percent for t in profitable_trades) / len(
                    profitable_trades
                )
                avg_loss = sum(t.pnl_percent for t in losing_trades) / len(
                    losing_trades
                )
        ...
        try:
            ...
            for trade_data in raw_trades:
                ...
                outcome = TradeOutcome(
                    ...
                    duration_hours=(
                        trade_data.get("hold_duration_minutes", 0) / 60.0
                        if trade_data.get("hold_duration_minutes")
                        else 0
                    ),
                    original_analysis=trade_data.get("original_analysis", {}),
                    market_conditions_entry=trade_data.get(
        ...
        try:
            for insight in insights:
                ...
                insight_data = {
                    ...
                    "news_context": getattr(
                        insight, "news_context", None
                ...
                self.database_bot.store_reflection_insight(
                    ...
                    market_conditions=insight_data.get("market_context")
                    or insight_data.get("news_context")
                    or "",
            ...
            logger.info(
                f"Stored {len(insights)} reflection insights in database (news context included where available)"
            )
        except Exception as e:
    ...
    def analyze_and_store(self, trade_outcome) -> List[ReflectionInsight]:
        ...
        if not insights:
            logger.info(
                f"No insights generated for {trade_outcome.symbol}, nothing to store."
            )
            return []

        # Validate persistence: retrieve immediately after storing
        persisted_insights = self.get_insights_for_symbol(
            trade_outcome.symbol, limit=10
        )
        if persisted_insights:
            logger.info(
                f"Validated persistence: {len(persisted_insights)} insights found for {trade_outcome.symbol} after storing."
            )
        else:
            logger.error(
                f"Persistence validation failed: No insights found for {trade_outcome.symbol} after storing."
            )
...
def selftest_reflection_bot():
    ...
    try:
                ...
                return [
                    {
                        "trade_id": "T1",
                        "symbol": symbol,
                        "ai_reflection": "Mocked insight",
                    }
                ]

            def store_reflection_insight(self, *a, **k):
                self.insights.append(a)
        ...
        assert isinstance(
            insights, list
        ), "analyze_completed_trade did not return a list."
        print("    -> Insight generation logic passed.")
        symbol_insights = bot.get_insights_for_symbol("AAPL", limit=1)
        assert (
            symbol_insights and symbol_insights[0].symbol == "AAPL"
        ), "get_insights_for_symbol failed."
        print("    -> get_insights_for_symbol logic passed.")
        print("--- ReflectionBot Self-Test PASSED ---")
