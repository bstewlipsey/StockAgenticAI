...
class VisualizerBot:
    ...
    def display_reflection_insights(self, symbol=None, limit=10):
        ...
        for i, insight in enumerate(insights, 1):
            if symbol:
                print(f"[{i}] {insight['timestamp']} | PnL: {insight['pnl_percentage']:.2f}% | {insight['key_insights']} | Lessons: {insight['lessons_learned']}")
            else:
                print(f"[{i}] {insight['symbol']} | AvgPnL: {insight['avg_pnl']:.2f}% | Trades: {insight['trade_count']} | Insights: {insight['combined_insights']}")
