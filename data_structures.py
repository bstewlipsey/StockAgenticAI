...
class TradingDecision:
    ...
    final_action: Optional[str] = (
        None  # Always set to signal.value (for test compatibility)
    )
...
class AssetAnalysisInput:
    ...
    news_sentiment: Optional[Dict[str, Any]] = (
        None  # e.g., {'overall_sentiment': 'positive', 'key_articles': [...]}
    )
    reflection_insights: Optional[List[str]] = None  # Relevant past reflections
    historical_ai_context: Optional[List[Dict[str, Any]]] = (
        None  # Past AI decisions/analyses for this symbol
    )
...
class AgenticBotError(Exception):
    """Custom exception for agentic bot errors. Use for critical, non-recoverable errors."""
